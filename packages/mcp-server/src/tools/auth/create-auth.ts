...
export const metadata: Metadata = {
  ...
  httpMethod: 'post',
  httpPath: '/auth',
  operationId: 'account-legacyAuthPost',
};

export const tool: Tool = {
  name: 'create_auth',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\n\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/auth_create_response',\n  $defs: {\n    auth_create_response: {\n      type: 'object',\n      properties: {\n        message: {\n          type: 'string',\n          description: 'Message of the authentication'\n        },\n        project: {\n          type: 'string',\n          description: 'Name of the authenticated project'\n        },\n        status: {\n          type: 'string',\n          description: 'Status of the authentication'\n        }\n      },\n      required: [        'message',\n        'project',\n        'status'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      jq_filter: {
...
export const handler = async (client: ModerationAPI, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.auth.create(body)));
  } catch (error) {
